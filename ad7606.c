...
{
	printk(KERN_DEBUG "In ad7606_reg_access  function -5\n");
...
{
	printk(KERN_DEBUG "In ad7606_read_samples  function\n");
	...
	if (st->gpio_frstdata) {
		printk(KERN_DEBUG "In gpio_frstdata -HIGH function\n");
		...
		data++;
		num--;
	printk(KERN_DEBUG "In ad7606_read_samples  num=%d\n",num);
	}
...
{
	printk(KERN_DEBUG "In ad7606_trigger_handler function \n");
...
{
	printk(KERN_DEBUG "In ad7606_scan_direct  function \n");
	//struct ad7606_state *st = iio_priv(indio_dev);
	struct ad7606_state *st = ad7606_get_data(indio_dev);
	int ret;
...
static int ad7606_read_raw(struct iio_dev *indio_dev,
			   ...
			   long m)
{
	printk(KERN_DEBUG "In ad7606_read_raw  function \n");
	int ret, ch = 0;
	//struct ad7606_state *st = iio_priv(indio_dev);
	struct ad7606_state *st = ad7606_get_data(indio_dev);
	...
	case IIO_CHAN_INFO_RAW:
		printk(KERN_DEBUG "In ad7606_read_raw  function -1 \n");
	...
	case IIO_CHAN_INFO_OVERSAMPLING_RATIO:
		printk(KERN_DEBUG "In ad7606_read_raw  function -2 \n");
		*val = st->oversampling;
		return IIO_VAL_INT;
	}
	return -EINVAL;
...
static int ad7606_write_raw(struct iio_dev *indio_dev,
			    ...
			    long mask)
{
	printk(KERN_DEBUG "In ad7606_write_raw  function  1\n");
	//struct ad7606_state *st = iio_priv(indio_dev);
	struct ad7606_state *st = ad7606_get_data(indio_dev);
	int i, ret, ch = 0;
	...
	case IIO_CHAN_INFO_SCALE:
		...
		return 0;
	case IIO_CHAN_INFO_OVERSAMPLING_RATIO:
		printk(KERN_DEBUG "In ad7606_write_raw  function  -2\n");
		if (val2)
			return -EINVAL;
...
static ssize_t ad7606_oversampling_ratio_avail(struct device *dev,
					       ...
					       char *buf)
{
	printk(KERN_DEBUG "In ad7606_oversampling_ratio_avail  function \n");
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
	struct ad7606_state *st = iio_priv(indio_dev);
...
{
	printk(KERN_DEBUG "In ad7606  request_gpios\n");
	...
	printk(KERN_DEBUG "In ad7606  request_gpios -1 st->gpio_convst:  %d\n",st->gpio_convst);

	st->gpio_reset = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_LOW);
	if (IS_ERR(st->gpio_reset))
		return PTR_ERR(st->gpio_reset);

	printk(KERN_DEBUG "In ad7606  request_gpios -1 st->gpio_reset:  %d\n",st->gpio_reset);
	st->gpio_range = devm_gpiod_get_optional(dev, "adi,range",
						 GPIOD_OUT_LOW);
	if (IS_ERR(st->gpio_range))
		return PTR_ERR(st->gpio_range);

	printk(KERN_DEBUG "In ad7606  request_gpios -1 st->gpio_range:  %d\n",st->gpio_range);
...
{
	printk(KERN_DEBUG "In ad7606 interrupt \n");
		struct iio_dev *indio_dev = dev_id;
	//struct ad7606_state *st = iio_priv(indio_dev);
	struct ad7606_state *st = ad7606_get_data(indio_dev);

	if (iio_buffer_enabled(indio_dev)) {
		printk(KERN_DEBUG "In ad7606  interrupt -1\n");
		gpiod_set_value(st->gpio_convst, 0);
		printk(KERN_DEBUG "In ad7606  interrupt -2\n");
...
{
	printk(KERN_DEBUG "In ad7606_validate_trigger   function \n");
	//struct ad7606_state *st = iio_priv(indio_dev);
	struct ad7606_state *st = ad7606_get_data(indio_dev);
	if (st->trig != trig)
		return -EINVAL;
...
static int ad7606_buffer_postenable(struct iio_dev *indio_dev)
{
	...
	int ret=0;
	unsigned int val;
	val =st->bops->reg_read(st, 0x02);
	ret =st->bops->reg_write(st, 0x02, ((val & 0xe7) | 0x10 ));
...
static int ad7606_buffer_predisable(struct iio_dev *indio_dev)
{
	...
	val =st->bops->reg_read(st, 0x02);
	ret =st->bops->reg_write(st, 0x02, (val & 0xe7) );
	return 0;
}
...
static void ad7606_regulator_disable(void *data)
{
	...
	printk(KERN_DEBUG "In ad7606  regulator disable function\n");

	regulator_disable(st->reg);
} 

static int ad7606_post_setup(struct iio_dev *indio_dev){
	printk("debug marker for testing adc_core");
...
static int ad7606_register_axi_adc(struct ad7606_state *st)
{
	...
	printk(KERN_DEBUG "In ad7606  probe entered ad7606_register_axi_adc -1\n");
...
static int ad7606_register(struct ad7606_state *st, struct iio_dev *indio_dev)
{
	...
	printk(KERN_DEBUG "In ad7606  probe entered dma_engine_buffer_alloc -1\n");
		 ...
		 const struct ad7606_bus_ops *bops)
{
	...
	if (ret) {
		printk(KERN_DEBUG "In ad7606  regulator enable failed case  \n");
