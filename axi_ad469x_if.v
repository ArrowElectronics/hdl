...
module axi_ad469x_if #(
  ...
  parameter[15:0]      ID = 0,         
  parameter[4:0]       NUM_OF_CHANNELS = 16,
  parameter[5:0]       DATA_BUS = 32,
  parameter[5:0]       ADC_RESOLUTION = 16,
  parameter[2:0]       NUM_OF_SDI = 2) (
 ...
 /* always @(negedge up_rstn or posedge up_clk) begin
  ...
  //assign   up_rdata = up_rdata_if_s | up_rdata_t;
    ...
    assign   up_wack = up_wack_if_s ;
  assign   up_rack = up_rack_if_s ;
  assign   up_rdata = up_rdata_if_s ;
  ...
  /*block for generating a mux and data based on CHannel ID of input ADC_Data	*/
		...
		//alligning data in sequence
		genvar i;
		generate
		for (i=0; i<DATA_WIDTH; i=i+1) begin : adc_data
			assign data_in_t[i*2+1:i*2]={data_in[i+DATA_BUS],data_in[i]};
		end endgenerate
		
		//seperating status bits and data bits and seperating data as per channel

		assign ADC_STATUS = data_in_t[status_length-1 : 0];
		assign CH_ID = {ADC_STATUS[3],ADC_STATUS[1],ADC_STATUS[2],ADC_STATUS[0]};
		...
		if (data_valid_t)begin
			ADC_DATA[CH_ID]=data_in_t[DATA_WIDTH-1:status_length];end
		end
 	...
 	/* config register blocks for AD469x*/
	  ...
	  always @(negedge up_rstn or posedge up_clk) begin
		...
		assign rx_cnv = (up_resetn && up_cnv_en)? pulse : 0;
     ...
     /*cpack data module	*/
		...
		.clk (spi_clk),
		.reset (up_rstn),
		.enable_0(enable_in[0]),
		.enable_1(enable_in[1]),
		.enable_2(enable_in[2]),
		.enable_3(enable_in[3]),
		.enable_4(enable_in[4]),
		.enable_5(enable_in[5]),
		.enable_6(enable_in[6]),
		.enable_7(enable_in[7]),
		.enable_8(enable_in[8]),
		.enable_9(enable_in[9]),
		.enable_10(enable_in[10]),
		.enable_11(enable_in[11]),
		.enable_12(enable_in[12]),
		.enable_13(enable_in[13]),
		.enable_14(enable_in[14]),
		.enable_15(enable_in[15]),
		.fifo_wr_en(adc_valid),
		.fifo_wr_overflow(adc_wr_overflow),
		.fifo_wr_data_0(data_temp[0]),
		.fifo_wr_data_1(data_temp[1]),
		.fifo_wr_data_2(data_temp[2]),
		.fifo_wr_data_3(data_temp[3]),
		.fifo_wr_data_4(data_temp[4]),
		.fifo_wr_data_5(data_temp[5]),
		.fifo_wr_data_6(data_temp[6]),
		.fifo_wr_data_7(data_temp[7]),
		.fifo_wr_data_8(data_temp[8]),
		.fifo_wr_data_9(data_temp[9]),
		.fifo_wr_data_10(data_temp[10]),
		.fifo_wr_data_11(data_temp[11]),	 
		.fifo_wr_data_12(data_temp[12]),
		.fifo_wr_data_13(data_temp[13]),
		.fifo_wr_data_14(data_temp[14]),
		.fifo_wr_data_15(data_temp[15]),
	  ...
	  wire [ADC_RESOLUTION-1:0] data_temp [NUM_OF_CHANNELS-1:0];
	  reg adc_valid;
	  ...
	  generate
			for (k=0; k<(NUM_OF_CHANNELS); k=k+1) begin :cpack
				assign data_temp[k]= adc_data_mode?(up_data_seq[k]? ADC_DATA[k] : 16'hDEAD) : ADC_DATA[k] ;
	  end endgenerate
